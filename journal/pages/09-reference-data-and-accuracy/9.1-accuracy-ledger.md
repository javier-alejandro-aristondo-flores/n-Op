...
| 2 | band_structure E_n(k) | RMS 50 meV within 2 eV of edges | TB-3NN-sp³d⁵ or Wannier/Fourier interpolation (`mesh-interpolation` sub-method, cheap); a **Wannierization quality gate** (band-reproduction error on held-out k-points) sets the mesh-σ floor. That floor is **per-composition, produced by the gate at compile time** — the interpolation error depends on the material and the mesh, so there is no constant to put in the tolerance ledger; it enters `combineTol` as a computed input, and a composition whose gate error exceeds its declared target is refused. Maximal localization can fail quietly on entangled conduction bands, which is what the gate exists to catch |
