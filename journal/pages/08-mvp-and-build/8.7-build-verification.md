...
3. The directory tree (Phase 0) contains every concept named in this plan and in
   `architecture.md`.
4. The nine regime extractions (`generic-dynamics`) are realizable as the
   compositions in `typed-compositions`.
...
7. The counts here match `canonical-vocabularies` exactly (12 methods, 20 templates,
...
1. **Registration sanity.** All 132 formulas instantiate as `ResidualGenerator`
   ...
   fixed-point-Jacobian conditioning check (`residual-machinery §5`);
   ...
   **Fidelity pairing.** Every generator whose lowering introduces representation
   error registers its paired fidelity generator (`residual-machinery
   §4.1`) — a non-`Dense` `CompressionPlan`, a truncated inner solve, or a
   ...
   (`compose-time-pipeline §4`) is the one lowering **exempt** by construction: it
