...
trains against. In the `PhysicsGraph` (`physics-graph`) they
...
## 1 The nineteen categories (a taxonomy facet)
...
and macro siblings added with `multiscale-state`, so the pre-multiscale
...
(`unified-state`), plus two cross-tier siblings (slow and macro,
`multiscale-state`):
  ...
  (the set of axis tuples emitted per `residual-definitions §2`) is
...
**Structural axes of GENERIC — 3 categories.**
  ...
  8. `Degeneracy` — `‖L δS/δx‖² + ‖M δE/δx‖²`. **Cert-only**: identically
     ...
     (`generic-dynamics`), so it is a generator-construction-bug tripwire, not a
 ...
 10. `Positivity` — `M ⪰ 0`, `f ∈ [0,1]`, `ρ ≥ 0`, `ω² ≥ 0`,
     ...
     spectral bounds on the `(Reciprocal, BlockDiag)` encoding (`gamma-hat`
     §5 — extreme eigenvalues per block are cheap); the T=0 idempotency
...
split by analytic kind):
 ...
 15. `Algebraic/MethodEquivalence` — different formulas claiming the
     ...
     **equivalence pair** binds two formulas that share an *agreement theorem* and
     ...
     `cert-obligations §1.2`; BTE-σ ≡ Kubo-σ); a
     ...
     **no** agreement theorem — Callaway/Slack κ vs iterative-LBTE κ, cheap-Chynoweth
     ...
     `τ_method` (`cert-obligations §1.2`), so a legitimate model gap is not scored as a bug.
...
Disjoint by the *type* of input the constraint reads:
 ...
 16. `Static/Snapshot` — depends only on the geometric + electronic
     ...
     Born stability (now `elastic-stability-criteria`); dynamical stability; space-group equivariance of
     the snapshot.
 17. `Static/Thermodynamic` — depends on snapshot + environment
     ...
     `d ln[D]/d(1/T)` vs `S_form` (`multiscale-state §13`).
...
(`multiscale-state §5, §10`). It appears in
`ContributionFacets.category` and nowhere else carries semantic weight.

## 2 The atomic unit: residual contribution
...
ContributionFacets =                         -- sidecar; not part of identity
  ( category : CategoryTag                   -- one of 19 symbolic tags (§1)
...
sense (`representation-substrate §3` row for cluster C5);
...
## 3 Examples of what becomes a separately-weightable contribution
...
- The conservation residual for one charge species in one slab
  subdomain.
- The Born stability (now `elastic-stability-criteria`) eigenvalue penalty per failing eigenmode of `C_ij`.
- The EOM violation per state-component
  `i ∈ {h, R_I, P_I, Π_h, γ̂, A}`, optionally per spatial / momentum bin.

Not "EOM violation"; not "Born stability" (the eponym for `elastic-stability-criteria`); not "conservation."
...
[0.60, 0.90)  Polish    — add Algebraic/MethodEquivalence + Static/Snapshot + Static/Thermodynamic
                          (Degeneracy is cert-only, §1 item 8 — never a training residual)
...
## 5 Granularity composes with hash-consing
...
— is the common case. Stage 3 (`compose-time-pipeline §3`) hash-consing
...
## 6 Closure (vocabulary count)

Residual contributions are **unbounded** (they unfold along the
generator's `axes`), but residual *generators* (`residual-machinery
§1`) remain countable: one per `(formula, applicability cell)` plus
the cert-only and ground-truth-bridge subtypes. The closed-vocabulary
discipline (`canonical-vocabularies`) holds at the generator level.

## 7 Per-residual error composition (the accuracy ledger)

Every `ResidualGenerator` (`residual-machinery §1`) carries a
...
error-model input that `typeclass-alphabet` `Quantity.combineTol` composes along the
...
`compose-time-pipeline §4`), **dressing staleness** (frozen Layer-1.25 one-shots), and
**coefficient-provenance `σ`** (`coupling-structure §8`) — so "is this
...
observables) live in [accuracy-ledger]; the reference battery (cert obligation 4, `cert-obligations §2`)
checks them at the MVP anchors. Every numeric tolerance named across `/physics` is valued once in the
**tolerance ledger** (`cert-obligations §1.2`), which is canonical for the list.
