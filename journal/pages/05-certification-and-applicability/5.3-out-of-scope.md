...
- Full NEGF tunneling, full SCPH/SSCHA, and the **live** iterative-LBTE / full
  ...
  ledger (`residual-definitions §7`, [accuracy-ledger]).
...
- **EDF-tail / hot-carrier breakdown above ~500 °C** — the BTE-full / full-band-MC
  ...
  corner is **cert-refused** (`coupling-structure §8`, `cert-obligations §1.3`),
...
- **Plasmon–phonon coupling / LST-relation breakdown at degenerate doping** — above
  ...
  **applicability-gates** the LST `ε_r` path and Fröhlich screening on `n < n_degenerate(host)`;
  ...
  degenerate-Einstein refinement, `multiscale-state §8.2`.)
- **III-N high-temperature thermal expansion** — QHA validity is **per-material and
  does not follow a Θ_D-scaled rule** (`[traps]` §40, which names this page as its
  enforcement site): diamond holds through ~800 °C, GaN and AlN both fail by ~500 °C,
  which is the design point. V1 has **no design-grade path**, only per-material σ-widening in the ledger
...
- Absolute Berry-phase / Wannier-center polarization (the λ-path `P_sp` evaluator) —
  deferred to V2. V1 uses the Z*-composition path (`coupling-structure`/registry rows 113–114, ±5%,
...
- **h-BN as a host material** — appears in research anchor sets only; the
  ...
  (purpose-and-scope). Decide in only if a 2D-substrate use case materializes.
