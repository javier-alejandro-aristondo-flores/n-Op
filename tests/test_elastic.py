...
The **published fit** in `data/diamond-strain-sweep/02-how-to-read-and-derive.md` §C2 gives
...
from physics.formulas.elastic import (
    ElasticFit,
    fit_elastic_constants,
    green_lagrange_voigt,
    reference_cell,
)
from physics.units import EV_PER_ANGSTROM3_TO_GPA
...
def sweep():
    from physics.ingest.sweep import read_all_families

    cache = Path(os.environ.get("NOP_SWEEP_CACHE", DEFAULT_CACHE))
    if not cache.is_dir():
        pytest.skip(f"no extracted sweep at {cache}; run tools/extract_sweep_family.sh")
...
def test_two_independent_routes_agree_on_the_bulk_modulus(sweep):
    ...
    """
    from physics.formulas.eos import fit_energy_volume
    from physics.ingest.sweep import (energy_volume_curve, pressure_volume_curve,
                                      read_family)
