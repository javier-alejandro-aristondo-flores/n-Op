...
def Test_The_Lift_And_Projection_Mix_Channels() -> None:
    ...
    assert numpy.asarray(lifted_field.values).shape == (5, 4, 4, 4)
    ...
    produced = numpy.asarray(projected.values)
...
def Test_The_Sensor_Encoder_Reads_Parameters() -> None:
    ...
    parameters = Coefficients(vector=numpy.asarray([0.1, -0.2, 0.3]), domain=CUBE)
    latent = encoder(parameters, PointSpec(numpy.zeros((1, 1))))
    assert numpy.asarray(latent.vector).shape == (8,)
...
def Test_The_Basis_Projection_Recovers_Exact_Coefficients() -> None:
    ...
    orthonormal, _ = numpy.linalg.qr(raw.T)
    basis_modes = orthonormal.T[:2]
    basis_mean = numpy.zeros(64)
    true_coefficients = numpy.asarray([1.5, -0.75])
    ...
    coefficients = encoder(field, PointSpec(numpy.zeros((1, 1))))
    assert numpy.allclose(numpy.asarray(coefficients.vector), true_coefficients, atol=1e-12)
...
def Test_The_Basis_Expansion_Queries_Anywhere() -> None:
    ...
    branch = Coefficients(vector=numpy.arange(6.0) / 6.0, domain=CUBE)
    ...
    grid_values = numpy.asarray(on_grid.values).reshape(-1)
    point_values = numpy.asarray(at_points.values).reshape(-1)
    assert numpy.allclose(grid_values, point_values, atol=1e-12)
...
def Test_The_Explicit_Stack_Chains_And_Inspects() -> None:
    ...
    def Halving_Local_Linear(values: Array) -> Array:
        return numpy.asarray(values) * 0.5
    ...
    assert numpy.asarray(produced.values).shape == (2, 4, 4, 4)
...
def Test_The_Wrappers_Enforce_Their_Laws() -> None:
    ...
    assert abs(float(numpy.asarray(balanced.values).mean())) < 1e-12

    renormalize = Conserving(FieldIdentity(), law="renormalize_to_electron_count")
    electron_count = Coefficients(vector=numpy.asarray([8.0]), domain=CUBE)
    conserved = renormalize(field, GridSpec((4, 4, 4)), electron_count)
    integral = float(numpy.asarray(conserved.values).sum()) * (8.0 / 64)
    ...
    class ZeroInner(FieldIdentity):
        ...
        def __call__(
            ...
            zeros = numpy.zeros_like(numpy.asarray(input_function.values))
    ...
    assert numpy.allclose(numpy.asarray(unchanged.values), numpy.asarray(field.values))

    conditioned = Conditioned(FieldIdentity(), channels=1, condition_width=2, seed=8)
    plain = conditioned(field, GridSpec((4, 4, 4)), None)
    assert numpy.allclose(numpy.asarray(plain.values), numpy.asarray(field.values))
    modulated = conditioned(field, GridSpec((4, 4, 4)), Coefficients(vector=numpy.asarray([1.0, -0.5]), domain=CUBE))
    assert not numpy.allclose(numpy.asarray(modulated.values), numpy.asarray(field.values))
