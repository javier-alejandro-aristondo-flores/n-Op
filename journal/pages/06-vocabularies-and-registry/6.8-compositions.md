...
Note also that four invocations above are **inline mathematics**
(`(2ω/c)·Im(√ε)`, `Re(√ε)`, `σ/(n·e)`, `F = E_BO + F_vib + F_el`), which
`[impl-04-formulas]` forbids outright — "no inline math". Registering the four
definitional rows named above is what retires them.
