...
PROBES: list[tuple[str, str, str, str, str, str]] = [
    ...
    ("out-of-vocabulary path", "data",
     ...
     "vocabulary"),
    # Shape, not content: one unescaped `|` turns a cell into three and the
    # table renders wrong with every by-name check still passing.
    ("unescaped pipe splitting a table row", "data",
     "journal/pages/06-vocabularies-and-registry/6.1-canonical-vocabularies.md",
     "| B9 | non-equilibrium-operating | L4 |",
     "| B9 | non-equilibrium-operating |E| | L4 |",
     "table-shape"),
    ("table row missing a cell", "data",
     "journal/pages/06-vocabularies-and-registry/6.1-canonical-vocabularies.md",
     "| B9 | non-equilibrium-operating | L4 |",
     "| B9 | L4 |",
     "table-shape"),
    # A band asserts a contiguous run exists; only its endpoints were checked,
    # so a hole in the middle read as fine.
    ("row band with a hole in the middle", "data",
     "physics/library/formulas/registry-manifest.csv",
     "\n3,effective-mass-tensor", "\n300,effective-mass-tensor",
