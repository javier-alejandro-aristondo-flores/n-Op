...
by this code. `data/diamond-strain-sweep/02-how-to-read-and-derive.md` carries a verified
...
from physics.formulas.eos import (
...
def series():
    from physics.ingest.sweep import read_family

    family = Path(os.environ.get("NOP_SWEEP_FAMILY", DEFAULT_FAMILY))
    if not family.is_dir():
        pytest.skip(
            f"no extracted strain family at {family}; run tools/extract_sweep_family.sh. "
...
def test_energy_route_reproduces_the_published_fit(series):
    ...
    from physics.ingest.sweep import energy_volume_curve
