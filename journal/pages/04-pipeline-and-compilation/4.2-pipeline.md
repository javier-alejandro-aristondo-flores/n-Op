...
**Soundness qualification.** "Exact and AD-safe" holds for the three rewrites as
listed — hash-consing, structural simplification, CSE — because none of them
changes the value or the derivative of the expression. It is **not** a property of
the rewrite *system*: once the rule set grows past these three, exactness needs a
side-condition discipline (positivity guards, branch-cut guards) that no rule here
carries, and **no rule-soundness contract exists**. That gap is named in
`[arch-18-open-decisions]`; `[traps]` §50 records the sharper form the evolver
needs, where a rewrite exact almost-everywhere but wrong on a measure-zero set is
tolerable for a pointwise scorer and not for a flow attracted to the bad set.
