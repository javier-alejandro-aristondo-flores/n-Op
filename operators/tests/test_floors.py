...
def Test_The_Hartree_Potential_Solves_A_Single_Mode() -> None:
    ...
    x = coordinates[:, None, None] * np.ones((1, extent, extent))
    wavenumber = 2.0 * np.pi / length
    density = 0.3 * np.cos(wavenumber * x)
    expected = 4.0 * np.pi * COULOMB_CONSTANT * 0.3 * np.cos(wavenumber * x) / wavenumber**2
...
def Test_Spectral_Derivatives_Match_Closed_Forms() -> None:
    ...
    x = coordinates[:, None, None] * np.ones((1, extent, extent))
    wavenumber = 2.0 * np.pi / length
    field = np.sin(wavenumber * x)
    gradient, laplacian = Spectral_Gradient_Magnitude_And_Laplacian(field, lattice)
    assert np.allclose(gradient, np.abs(wavenumber * np.cos(wavenumber * x)), atol=1e-9)
