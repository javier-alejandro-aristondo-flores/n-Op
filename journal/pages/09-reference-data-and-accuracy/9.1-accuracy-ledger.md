...
declared characteristic scale that `typeclass-alphabet Quantity.combineTol` composes into a
per-residual error budget (`residual-definitions §7`). It is a *reference table*, not the
...
| 49 | hot_carrier_distribution f(E,F) | shape ±20%; high-E tail decisive | **OUTSIDE registry** (full-band MC); V1 EDF-tail correction `Δα(E,T_L,T_e)` is **fit only to external anchors and frozen w.r.t. the PINO loss** (else circular) — and since no anchor data exist in V1, it **ships as identity** and the high-E×high-T corner stays cert-refused (`coupling-structure §8`, `cert-obligations §1.3`) |
| 50 | electromigration E_a^EM | ±0.1 eV | NEB interface hop |
| 51 | defect_evolution dN/dt | **factor-2** | master-equation / kMC (slow tier, `multiscale-state`) |
...
The reference battery (cert obligation 4, `cert-obligations §2`) checks these at the MVP anchors.
...
(`coupling-structure §8`); for the MVP diamond they are `curated`, other materials
...
electron-phonon ZPR; zero-point lattice expansion is row 63's job, `coupling-structure §8`):**
...
co-activation with row 63 on the same observable, `cert-obligations §1.3` — so the tag must describe what
...
`cert-obligations §1.3` / `coupling-structure §8`):*
...
(`residual-definitions §7`); `Quantity.combineTol` composes them along the DAG (max-abs or RSS per
`typeclass-alphabet`). Stage-4 `CompressionPlan` (LowRank/HODLR/TT) truncation carries a **per-plan error
target** that also enters `combineTol` (`compose-time-pipeline §4`), so the emitted residual's budget
