...
class PointwiseProjection(Operator[GridFunction, GridFunction]):
    ...
    def __init__(self, output_channels: int, hidden_channels: int, bounded: bool = False, seed: int = 0) -> None:
        ...
        scale = numpy.sqrt(2.0 / (output_channels + hidden_channels))
        ...
        self.parameter_values: dict[str, NDArray[numpy.float64]] = {
            ...
            "projection_biases": numpy.zeros(output_channels),
        }
...
class BasisExpansion(Operator[Coefficients, Representation]):
    ...
    def __init__(self, latent_width: int, trunk_widths: tuple[int, ...], fourier_orders: int = 4, seed: int = 0) -> None:
        ...
        self.last_trunk_features: NDArray[numpy.float64] | None = None


    def Coordinate_Features(self, points: NDArray[numpy.float64]) -> NDArray[numpy.float64]:
        ...
        for order in range(1, self.fourier_orders + 1):
            angle = 2.0 * numpy.pi * order * points
            feature_blocks.append(numpy.cos(angle))
            feature_blocks.append(numpy.sin(angle))
        return numpy.concatenate(feature_blocks, axis=1)
    ...
    def __call__(
        ...
        branch_vector = numpy.asarray(input_function.vector, dtype=numpy.float64)
        produced = numpy.asarray(
            self.Forward(self.parameter_values, branch_vector, trunk_features), dtype=numpy.float64
        )
        if isinstance(output_discretization, GridSpec):
            shape = output_discretization.shape
            point_count = shape[0] * shape[1] * shape[2]
            quadrature = UniformGridQuadrature(abs(float(numpy.linalg.det(numpy.asarray(input_function.domain.lattice)))), point_count)
