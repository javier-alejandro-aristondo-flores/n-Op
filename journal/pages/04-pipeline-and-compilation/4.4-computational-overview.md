...
- **diff-tag:** `D0` pure read (1) · `DN` no useful derivative — integer / categorical (6) · `D1` smooth, gradient direct (91) · `D2` adjoint-required-and-gated (26) · `D3` implicit-function adjoint (2) · `D4` relaxed, e.g. log-sum-exp soft-hull / Gumbel-Softmax (6). Vocabulary owned by `[impl-04-formulas]`.
