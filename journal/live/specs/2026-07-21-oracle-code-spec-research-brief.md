...
3. **Every *targetable* observable is on the differentiable path** (baked adjoint → cotangents), carries a σ, and is pinnable as a target — not a forward-only readout. In the current differentiability vocabulary (`D0 | DX | D1 | D2 | D3 | D4`, owned by `impl-04-formulas`) this means **targetable ⇒ `D1`/`D2`/`D3`**; `DX` rows (no useful derivative — integer, categorical, boolean, set-valued) cannot be differentiable targets at all, and `D4` rows (relaxed: argmin / hull / sort / discrete) are targetable only through their declared relaxation.
