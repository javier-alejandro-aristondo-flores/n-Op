...
> residual-driving grouping is the 11 physics-domain bundles (canonical-vocabularies §4). Row-#s here are
...
|---|---|---|---|---|
| **Diamond** | Pristine C (sp³, Fd-3m) | Indirect gap ~5.47 eV; phonon spectrum extends to ~165 meV (highest of any solid → Debye T_D ≈ 2200 K); strong electron–phonon coupling but anharmonicity is small below 1500 K; no native shallow donor | Active semiconductor channel; thermal spreader | **Air-oxidation onset ~600–700 °C is the lifetime limiter**; sp³→sp² graphitization only above ~1500 °C in vacuum ([traps] §43 — conflating the two is an ~800 °C error in the operating ceiling). H-termination desorbs around 700–900 °C → loss of 2D hole gas |
...
| 13 | `thermal_conductivity κ(T)` (tensor) | TR/TN | BTE-iterative on 12 | `κ_αβ = Σ_λ c_v,λ v_α v_β τ_λ` | (a) Slack formula; (b) Cahill minimum-κ for amorphous limit | `r_κ = κ_predicted − Σ_λ c_v,λ v_α v_β τ_λ` | **±20% @300 K, ±25% @773 K, ±35%→±15% @1100 K with the 4-phonon correction** ([accuracy-ledger] is canonical) | Diamond κ ∝ T⁻¹·² above 200 K; **2200 W/mK at 300 K** (anchored to `κ_iter`, not `κ_RTA` — RTA/3-phonon underestimates by 30–50% near 300 K), ~700 at 600 K, ~400 at 900 K | 9, 11, 12, FoMs |
| 14 | `thermal_expansion_tensor α_αβ(T)` | TR/TN | QHA on 9, 11 | `α = (1/B) Σ_λ γ_λ c_v,λ` (Grüneisen-Mie) | (a) Constant γ_avg in QHA; (b) Empirical Debye fit | `r_α = α − (1/B(T)) Σ_λ γ_λ c_v,λ(T)` | ±10% | QHA validity is per-material, not Θ_D-scaled ([traps] §40): diamond holds through ~800 °C (0.49 Θ_D), **GaN fails near 500 °C (1.29 Θ_D)**, AlN also at 500 °C (0.77 Θ_D) — no single fraction fits | 9, 11, 13, FoM mismatch |
...
| 31 | `schottky_barrier_height φ_B(metal/semi, T)` | SC | NEGF or DFT slab + alignment; **interface-MIGS calc may be OUTSIDE registry if NEGF not present** → landed: registry row 80 (NEGF-transmission) | `φ_B = φ_M − χ_S − (1 − S)(φ_M − φ_CNL)`, equivalently `φ_B = φ_CNL + S(φ_M − φ_CNL) − χ_S` (Cowley-Sze interpolation; **S=1 → Schottky-Mott (now `barrier-from-workfunction-affinity`), S=0 → Bardeen/fully pinned**, matching [deriv-defects] §F.3. The two forms are algebraically identical — expanding the first gives the second exactly — so neither inverts the other; a note here claiming the `S(φ_M − φ_CNL)` form "inverts it" was removed 2026-07-22, having survived as residue of a genuine pinning-parameter inversion fixed elsewhere) | (a) Schottky-Mott `φ_B = φ_M − χ`; (b) Cowley-Sze with tabulated S | `r_φB = φ_B − [φ_CNL + S(φ_M − φ_CNL) − χ ± dipole correction]` from slab | ±0.1 eV (critical: 100 meV → ×e^4 contact resistance at 500°C) | Interface chemistry (carbide formation) shifts φ_B with thermal soak | 7, 8, 30, FoM ρ_c |
...
| **QHA breakdown at high T** | GaN (Θ_D ≈ 600 K) fails near 500°C; AlN (Θ_D ≈ 1000 K [Wave-1 A10]) also fails at 500°C — the III-N no-design-grade-path hole, out-of-scope. **Per-material, not Θ_D-scaled** (`[traps]` §40): those three failure points sit at 0.49, 1.29 and 0.77 of Θ_D, so no single fraction fits, and the "Θ_D/2" justification this row used to give is the one that register explicitly retired; diamond (~2200 K) safe through 800°C | 10–30% error in α, c_v | self-consistent phonon (SCPH) or stochastic-self-consistent harmonic (SSCHA) — outside current registry → landed: registry row 13 (live solve V2 per out-of-scope) |
| **Bardeen MIGS vs Schottky-Mott (now `barrier-from-workfunction-affinity`)** for φ_B | every metal/UWBG contact; especially W/diamond, Mo/diamond | 0.3–1 eV uncertainty between extremes | NEGF slab with chemical interface; Cowley-Sze interpolation as analytic residual |
...
Observables/residuals that the closed registry as of this catalog's first draft (then 12 method entries × 12 template entries × 25 formula entries (the braced list below has 25 members; "24" was the figure this preamble carried); since grown to 12 methods + 3 sub-methods, 20 templates, and 132 substantive formulas — `canonical-vocabularies §3`) did **not** cover, and that downstream streams had to add or document-as-skipped. Kept as the scoping record; each bullet carries its disposition:
...
- **Multiphonon emission / Huang-Rhys / Marcus-crossing formula** — needed for 29 (capture cross-section) → landed: registry rows 40–41 (multiphonon-emission-capture, lattice-relaxation-phonon-quanta).
...
- **Self-consistent phonon / SSCHA** for non-diamond materials at ≥500°C where QHA fails → landed: registry row 13 (SCPH-self-consistent-phonons; live solve V2 per out-of-scope).
...
**Catalog total: 52 catalog observables across the 8 data-shape groups above** (canonical bundle grouping: the 11 physics-domain bundles, canonical-vocabularies §4). Diamond-relevant observables flagged in Part A; centrality ranking in Part D; honest-limits and registry-gap flags in Parts E & F together form the scoping handoff to downstream streams.

---

## Changelog

- **2026-07-21 (P1 repair):** the κ(T) accuracy regime in row 13 was rewritten to the
  ledger's per-temperature bands. It read "±10% diamond, ±20% III-N" with a 2000 W/mK
  300 K anchor — both superseded, and [accuracy-ledger] cites *this file* as its source,
  so the disagreement pointed the wrong way. Old values, for the record: ±10% / ±20%,
  "drops from 2000 to ~600 W/mK at 800 K". Three literature names in the Part-B
  vocabulary list gained their landed registry names.
...
- **2026-07-16 (cert round 1):** Part-F gap-summary brought to current truth: the live-tense
  framing line quoting the registry as 12 methods x 12 templates x 24 formulas became
...
- **2026-07-16 (strata rewrite):** status banner converted to this changelog and its content
  ...
  - Row #42 displacement threshold: **"~25 eV Ga in GaN" → "~20 eV Ga-sublattice in GaN
    ...
    `[timeline] §2026-06-10 (re-audit addendum)` §1; multiscale-state §4 F-H1 carries ~20 eV).
  - Stale "OUTSIDE registry" flags annotated in place with their landing rows (flag text kept as
    ...
    105–108; #52 → row 115; Part E SCPH → row 13 (live solve V2 per out-of-scope); Part E 4-phonon
  ...
  - Closing line reworded to the catalog-observable vocabulary (52 catalog observables across the
    8 data-shape groups; canonical grouping = the 11 physics-domain bundles, canonical-vocabularies §4).
