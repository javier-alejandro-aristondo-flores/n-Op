...
def Frequency_Split_Relative_L2(prediction: Field, truth: Field, cutoff_modes: float) -> tuple[float, float]:
    ...
    return Band_Error(low), Band_Error(~low)
...
def Structural_Similarity_3d(prediction: Field, truth: Field, window: int = 7, data_range: float = 1.0) -> float:
    ...
    mean_one = Periodic_Box_Mean(first, window)
    mean_two = Periodic_Box_Mean(second, window)
    variance_one = Periodic_Box_Mean(first * first, window) - mean_one * mean_one
    variance_two = Periodic_Box_Mean(second * second, window) - mean_two * mean_two
    covariance = Periodic_Box_Mean(first * second, window) - mean_one * mean_two
...
def Wasserstein_1d(prediction: NDArray[np.float64], truth: NDArray[np.float64], spacing: float) -> float:
    ...
    first = prediction / (np.sum(prediction) * spacing)
    second = truth / (np.sum(truth) * spacing)
    cumulative_gap = np.cumsum(first - second) * spacing
