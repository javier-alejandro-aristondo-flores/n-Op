...
| `Path` | **Retired column.** Under the always-cheap pipeline (see [arch-07-pipeline]), every formula is on the single residual surface; the `cheap`/`faithful` distinction has been collapsed. The column survives in the CSV for historical row identity only. |
...
`D0 | DX | D1 | D2 | D3 | D4` vocabulary. Restating it here is what let three
