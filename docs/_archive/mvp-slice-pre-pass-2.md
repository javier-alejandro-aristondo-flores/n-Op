...
This is the **buildable first unit**: the minimal projection of the comprehensive
spec (`architecture.md`, `implementation-plan.md`) onto the diamond MVP. The full
spec stays the target; this document says *what to build first*. Everything here
is a strict subset of the canonical model — it selects and parameterizes, it does
not add or contradict.

> **Interpretation note.** "Electron-cloud diffusion through the lattice" is read
> here as **charge / carrier transport** — electrons moving through the lattice,
> the electronic parallel to heat diffusion — i.e. the electronic-structure
> substrate plus carrier-transport dynamics. The alternative reading is *real-time
> evolution of the electron density `γ̂` itself* (L1 Liouville dynamics rather than
> L4 transport). If the latter is intended, Cap 2 below shifts accordingly; flag
> it and this doc is revised.
...
## 2. The γ̂ budget (closes the feasibility premise at MVP scale)
...
- **Sizing (primitive cell, faithful path):** PW cutoff ~400 eV ⇒ `N_PW ≈ 1000`;
...
- **Cheap path:** tight-binding **3NN sp³d⁵** for carbon ⇒ a `~18 × 18` Hamiltonian
  per k — kilobytes.
...
## 3. The three capability slices

Each capability is a strict selection from the closed vocabularies of
`architecture.md §8` (methods, templates, formulas, bundles), the residual
categories of §11, and the cert obligations of §12. Formula numbers reference
...
| cheap / faithful | TB 3NN E_BO + closed-form C_ij / DFT E_BO + DFPT-stress C_ij |
...
| cheap / faithful | TB bands + Matthiessen/Caughey-Thomas μ + drift-diffusion / DFT+G₀W₀ bands + BTE-RTA |
...
| cheap / faithful | QHA phonons + Callaway κ (closed-form) / DFPT phonons + 3-phonon BTE-RTA |

---

## 4. In-MVP vs deferred
...
- Layers 1 + 1.25 (G₀W₀, QHA, DFPT). The two-tier cheap/faithful split per
  capability.
...
## 5. Closed-form vs kinetic — the MVP discipline holds

Two of the three capabilities (carrier and heat diffusion) are intrinsically
kinetic. The two-tier accuracy split resolves the tension with the "as much
closed-form as possible" discipline:

| Capability | Cheap path (closed-form, label generation) | Faithful path (kinetic, residual loss) |
|---|---|---|
| Structure | TB 3NN E_BO; closed-form C_ij averages | DFT E_BO; DFPT stress |
| Carrier diffusion | TB bands; Matthiessen + Caughey-Thomas μ; drift-diffusion | DFT+G₀W₀ bands; BTE-RTA with e-ph `g²` |
| Heat diffusion | QHA phonons; Callaway κ(τ) | DFPT phonons; 3-phonon BTE-RTA |

The cheap path is genuinely closed-form; the kinetic machinery (BTE, DFPT) is
reserved for the faithful residuals the PINO trains against. No conflict.

---

## 6. Decisions this slice forces (feeds the audit's open holes)

- **Implementation language (H1).** Now decidable against concrete needs:
  reverse-mode autodiff through the BTE-RTA adjoint and the orbital / tight-binding
  linear algebra, and GPU support for k-point meshes. (These point at an
  autodiff-first numerical stack.)
- **Cheap E_BO surrogate.** Tight-binding 3NN sp³d⁵ for carbon — named here so the
  cheap path is concrete.
- **Layer-1.25 substrate data (H7).** The closed-form premise needs L1 to expose
  more than `γ̂`: G₀W₀ needs ~30–50 **unoccupied bands + wavefunctions**; QHA needs
  **volume-dependent (Grüneisen) phonons**. These are the L1 outputs the MVP
  requires — specify them when building `state/level-1`.
- **Reference-battery seed (H4).** Seed `physics/library/cert/reference-data/` with
  the ~10 diamond rows the MVP validates against: lattice a, indirect gap, C₁₁/C₁₂/
  C₄₄, Debye T, κ(300 K), max phonon energy, cohesive/formation energy, and the
  diamond–graphite boundary point.

---

## 7. MVP build order

A focused subset of the phases in `implementation-plan.md §14`:
...
6. canonicals — `E_BO` (TB cheap / DFT faithful) and the phonon Hessian.
...
Completing this slice yields a diamond-only `/physics` that can generate labels,
emit faithful residuals, and certify them for all three capabilities — the
concrete substrate `/informed-operator` then trains against.
