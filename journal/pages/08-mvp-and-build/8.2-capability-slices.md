...
`canonical-vocabularies` (methods, templates, formulas, bundles), the residual
categories of `residual-definitions`, and the cert obligations of `cert-obligations`.
...
| Formulas | **57** elastic-stability-criteria · **60** elastic-constants-Cij · **61** bulk-modulus · **62** sound-velocity-isotropic · **85** structure-uniqueness-CSP · **30** defect-formation-energy · **44** surface-grand-potential-γ (B5) · **52** alloy-lattice-interpolation · **54** critical-thickness-force-balance (c-BN/diamond hetero) · **67** phase-diagram-convex-hull · **124** tp-aware-hull (δ_meta metastability band — metastable diamond reads R_hull = 0) |
...
| Implementation | DFT `E_BO` + DFPT-stress `C_ij`; TB 3NN sp³d⁵ as SCF warm-start initializer (`forced-decisions`) |
...
relaxation, space-group equivariance for a broken symmetry, Born stability (now `elastic-stability-criteria`) for an over-stretch.
...
| Formulas | **1** bandgap-direct · **2** bandgap-indirect · **3** effective-mass-tensor · **4** DOS-tetrahedron · **5** fermi-level-charge-neutral · **6** quasi-particle-shift-G0W0-surrogate · **14** drude-conductivity · **15** matthiessen-mobility · **16** field-dependent-mobility · **18** v-sat-intervalley · **19** hall-mobility-from-σ · **20** mobility-impurity-phonon · **24** electronic-kappa-from-conductivity |
| Excluded (non-polar) | **17** v-sat-POP-limit · **21** polar-coupling-constant · **22** polar-optical-scattering-rate — masked off by the `is-polar-material` applicability classifier (false for diamond) |
...
| Formulas | **7** acoustic-sum-rule · **8** dynamical-matrix-hermiticity · **9** phonon-dispersion · **10** phonon-DOS · **11** phonon-group-velocity · **12** grueneisen-mode (QHA / thermal expansion) · **25** single-mode-rta-lattice-kappa · **121/122** high-T κ siblings of row 25 (4-phonon factor + dormant iterative-LBTE consistency pair) · **70** self-heating-T_op (B9; cheap closure) |
...
| Implementation | DFPT phonons + 3-phonon BTE-RTA; the closed-form QHA + Slack/Callaway κ sits alongside it as a **consistency pair**, not an equivalence pair — the two share no agreement theorem, only a bounded model gap, so obligation-6 trips on excess beyond the declared `τ_method` rather than on any disagreement (`[cross-cutting-rules]`, `[traps]` §29). Calling it a method-equivalence would score the legitimate model gap as a bug. |
