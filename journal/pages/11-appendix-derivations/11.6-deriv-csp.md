...
All "typed signatures" below use the convention `name : (input-types) -> output-type [cost-tier, differentiability]`, with cost tiers `T0` (closed-form, ~µs), `T1` (small linear algebra / sums, ~ms), `T2` (ML-potential single-point, ~10ms–1s), `T3` (DFT-single-point, minutes–hours), `T4` (full self-consistent loop, hours–days). Differentiability ratings in this survey are its own three-way split, written as words — `smooth` (analytic), `piecewise` (piecewise smooth, subgradient OK), `combinatorial` (needs surrogate / softening). They were originally spelled `D+` / `D0` / `D-`, which collided head-on with the registry's `D0 | DN | D1 | D2 | D3 | D4` tags — and `D0` meant the *opposite* of what it means there (piecewise-smooth here, pure-read there). None of these is a registry tag; `[named-formulas]` is canonical for those. The cost tiers below likewise run `T0`–`T4`, one tier deeper than the registry's `T0`–`T3`.
...
with `k_r`, `k_θ` from a tabulated bond-angle force field. The Hessian eigenvalues at u=0 give a 3N×3N stiffness; negative eigenvalues → soft modes. This is T1 (matrix diagonalization of a sparse 3N×3N) and `smooth` because the Hessian is an analytic function of the structure. It gets the **sign** of stability right ~85% of the time on benchmark sets (vs full phonons), which is enough for label generation; full phonons stay in the faithful path.
...
|---|---|---|---|---|---|
| `R_LatticeMatch` | `(a_A, a_B, m, n, θ) -> R⁺` | `((m·a_A·cos θ − n·a_B) / (n·a_B))²` averaged over 2D | Anisotropic with full strain tensor ε_αβ; Vegard correction (now `alloy-lattice-interpolation`) | T0 | smooth |
| `R_StrainEnergy` | `(ε_αβ, C_ijkl) -> R⁺` | `½ V · C_ijkl · ε_ij · ε_kl` (linear elasticity) | DFT relax of strained cell | T0 / T3 | smooth |
| `R_WorkFunctionAlign` | `(Φ_metal, χ_semi, Δ_dipole) -> R⁺` | Schottky-Mott (now `barrier-from-workfunction-affinity`): `Φ_B = Φ_metal − χ_semi`; residual = deviation from target | Slab DFT with explicit dipole correction | T0 / T3 | smooth |
...
> **Stability ceilings defer to [deriv-defects] §F.4**, which owns the diamond/metal
> contact table. Where a `T_stable` cell here disagreed with it, F.4 wins; the two drifted
> systematically because this table restated rather than cited. Provenance for all of them
> is still pending the metals wave ([traps] §51).

| Metal | Φ (eV) | Diamond contact behavior | Carbide? | T_stable | Use case |
|---|---|---|---|---|---|
| W | 4.55 | Schottky on undoped, near-Ohmic on heavily B-doped; W₂C/WC growth measurable from ~600–700°C (kinetics: defects file F.5 — ~3 nm per 1000 h at 500°C) | Yes (WC, W₂C) | see [deriv-defects] §F.4 (~600 °C, carbide-limited) | Gate metal, refractory contact |
...
| WSi₂ | 4.7 | Silicide contact; needs SiC interlayer typically | Indirect | Good to 800°C | Specialized |
...
The current 5 categories (EOM-violation, degeneracy, conservation, positivity, algebraic-identities) are dynamical-physics residuals — they constrain trajectories of x(t). They don't naturally express **discrete-structure validity** ("does this candidate satisfy Hume-Rothery (now `substitution-compatibility-score`)?") or **equilibrium-population consistency** ("does [V] = exp(-E_f/kT)?").

Proposed two new categories:

6. **`structural-validity`** — Pauling, Hume-Rothery (now `substitution-compatibility-score`), charge balance, stoichiometry, symmetry consistency, bond-length sanity, coordination consistency, Born stability (now `elastic-stability-criteria`), cheap-phonon stability. All are **algebraic constraints on the static structure** rather than on dynamics. They are mostly `smooth`, cheap, and fit the PINO loss directly.
...
3. **Band alignment** (`band-alignment` bundle): `Φ_B^n = Φ_W − χ_diamond(100, clean) = 4.55 − 0.5 = 4.05 eV` (Schottky-Mott, registered as `barrier-from-workfunction-affinity` cheap path); PINO refines. **χ is termination-tagged** ([traps] §18, and this file states the rule in Part B): the bare 0.7 used here previously is a (110)-clean value attached to a (100) interface, and H-terminated (100) is χ ≈ −1.3 eV ⇒ 5.85 eV.
...
**Summary recommendation to the conductor.** The minimum viable expansion of `/physics` for UWBG-chip CSP support is: **three new methods, four new templates, fifteen new named formulas, four new observable bundles, two new residual categories**, plus a `reference-phase-energy-cache` primitive. All net additions preserve the GENERIC dx/dt structure (validity/thermo residuals are static constraints on the state x, not modifications to L or M). Cheap-compute paths for all proposed residuals stay in T0/T1 and are end-to-end differentiable, suitable for direct inclusion in the PINO physics loss. Diamond-metal interface chemistry (Part C.3 table) is the most domain-specific content and should be encoded as a learned-feature table inside the PINO with `R_InterfaceReactivity` as the supervising residual.

---

## Changelog

- **2026-07-16 (cert round 2):** Born-stability (now `elastic-stability-criteria`) worked example C_ij normalized to the seeded
