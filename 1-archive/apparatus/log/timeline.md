...
### 2026-08-07 · The specification and the implementation become twin trees, and six path mechanisms turn out to be load-bearing

`journals/` was organized by journal, section and page; `physics/` and
`informed-operator/` were organized by whatever each author reached for. The
correspondence existed — the `libraries:` map declares it and the checker stamps it onto
every page in the emitted index — but it sat 160 lines below a diagram that named
different directories, and the audit had already recorded that as a measured navigation
failure with a standing recommendation to fold the mapping into the diagram. That
recommendation is now applied.

The trees are `journals/`, `programs/` and `tests/`, mirrored section for section, with
`apparatus/` holding everything that checks or feeds them. The journal `interface` is
renamed `loops`, so the journal and the library it specifies stop carrying two names.

**The mirror is section-level and deliberately unenforced, because the two sides
decompose on different axes.** `journals/operator/` describes anatomy — seam, structure,
training, loss. The operator library decomposes by data-and-experiment lifecycle — roots,
data, symmetry, observables, baselines, train, evaluate, exam. Filing one into the other
would assert correspondences the corpus does not state, so a section directory appears on
the implementation side only where the twin is real, and the rest sits flat. Two sections
were honest to create: `seam/` for the one file a page governs explicitly, and
`training/`. `structure/` is created **empty**, against five pages, and the empty
directory is the point.

`units.py` stays flat at the library root. Its own docstring says it implements a gap
rather than a page — no unit system is declared anywhere in the specification — so there
is no section that could honestly claim it.

**Six mechanisms broke on the move, and the reason to record them is that they are
invisible when correct.** The three checkers derived the repository root as
`__file__.parent.parent`, which after the move resolved one level short and pointed
`journals/` at a directory that does not exist. `locations.py` walked one level up and
asserts on Gate 4's results *at import time*, so a wrong walk makes the whole operator
library unimportable rather than merely wrong. Gate 4's `paths.py` walked three levels and
now walks four. The licensing scanner named its guarded tree as the bare string
`feasibility`. The path-citation sweep hardcoded five top-level directory names, four of
which moved.

**One correction to an earlier assessment in this record's own idiom.** The licensing
scanner was first judged the worst silent-failure site of the six — a scanner that walks a
missing directory, finds nothing and exits 0, on a public remote guarding licensed
pseudopotentials. That is wrong, and the tool is better built than the assessment credited:
it plants its calibration probe *inside* the guarded tree, so a stale path raises
`FileNotFoundError` with no handler, and an empty tree is a fatal `return 1`. It cannot
report clean while scanning nothing. The edit was still required; it belongs in the loud
column. What the exit code does *not* protect is coverage shrinking silently, so the
scanned file count is now the quantity compared across the move: 248 before, 248 after.

**The path-citation sweep is a superset rather than a replacement.** Narrowing it to the
four new top-level names would have stopped it catching legacy-form citations such as
`data/registry-manifest.csv`, and the calibration caught exactly that: one probe went from
caught to missed. The pattern now carries the new names and the old ones together, so a
page naming either form is flagged.

**Python's standard library forced one deviation from the approved plan.** The plan put
`programs/` on the import path, making the import `oracle.registry.elastic`. That places a
directory named `operator` on the path, and `operator` is a path-resolved standard-library
module rather than a frozen one — `pandas` imports it from `frame.py` and `series.py`, so
the shadowing would break pandas from the inside, at import time, silently. The import is
therefore `programs.oracle.registry.elastic` through a `programs` package, and no
top-level `operator` name is ever created.

A root `conftest.py` now puts the repository root on the path. This fixes a fragility that
predates the move: the suite resolved its imports only under `python -m pytest` from the
root, a bare `pytest` failed with `ModuleNotFoundError`, and the contract was written down
nowhere. Both forms now pass.

- **Evidence** — `python apparatus/tools/check_structure.py` at 50 pages, 302 owned topics,
  62 open questions; `check_the_checker.py` at 34 probes caught, 0 missed, 0 error sites
  unreached; `check_no_licensed_content.py` at 248 files scanned with its calibration
  passing; 42 tests passing under both `python -m pytest` and bare `pytest`; the operator
  library's five must-fire checks, including the capacity check at 35.42 meV against the
  72.82 meV baseline the model contains. `git diff --cached -M` reports 358 renames, 8
  additions and **0 deletions**. Three restructure-specific defects were planted and each
  was watched to fail: a stale `GUARDED` (`FileNotFoundError`, exit 1), a reverted root walk
  (`fatal: no contract at journals/practice/agent-contract.md`), and a bare path citation on
  a non-exempt page (caught by name). Three of three.
- **Attribution** — the twin-directory session.
- **Supersedes** — the position that the journal-to-library correspondence could live in a
  YAML map 160 lines from the diagram that contradicted it. Also the README's claim that two
  of the three libraries are empty: the operator library learns strain → HSE06(0.27) for
  diamond and beats the baseline it contains, and only the loops library is empty. Also the
  claim, made in this session and corrected in it, that the licensing scanner could pass
  while scanning a directory that no longer exists.

**MISSING: what the `§`-numbered specification is.** The operator library's modules carry
thirty references across ten files to sections 3, 4.4, 5, 6.2, 6.4, 6.5, 6.6, 7.1, 7.6 and
8 of a document that is not in this base. They are legitimate external citations under the
citation rule and they resolve to nothing a reader here can open. None was converted to a
corpus citation, because converting one would require verifying it against a document that
cannot be read, and a plausible guess landed in this corpus is a fabricated citation. The
gap is declared as an open question on `learnable-structure-contract` instead. Zero
converted, thirty left.

**Not swept:** `apparatus/audit/` references `data/…` paths that are now
`apparatus/data/…`. The audit is a historical record outside the checker's scope, and its
`journals/` references remain correct because `journals/` did not move. The two
`properties#catalogue` anchor references in the audit *were* updated, because an anchor
that no longer resolves would break the grounding procedure the audit depends on.
