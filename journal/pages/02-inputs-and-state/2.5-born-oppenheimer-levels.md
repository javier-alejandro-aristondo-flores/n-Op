...
The 7-tuple state (`unified-state`) partitions into four levels whose
...
(not competing with) the `PhysicsGraph` (`physics-graph`),
...
`multiscale-state`. The micro 7-tuple (`unified-state`) is the L1/L2 tier;
this resolves the apparent tension with `unified-state`'s emergence wording (see
`unified-state` and `multiscale-state §1`).

In the `PhysicsGraph`, BO level is **derivable** from the transitive
inputs of a node — it is not a stored field on `Node`
(`physics-graph §5`). Stage 1 ordering follows the level
discipline: L1 nodes are constructed first; L2/L3/L4 nodes depend on
their L1/L2/L3 ancestors.

## 1 Dressing tiers (V1 vs V2 scope)
...
Layer 1.25   One-shot closed-form dressing — pure functions, no iteration.
             ...
             invariants.
             Cert: OneShotCert (residual-machinery §7).
             **Frozen at reference (normative).** A Layer-1.25 dressing is
             computed ONCE per composition, at the reference state, and is
             thereafter constant. It does not respond to the PINO's state
             excursions and contributes no gradient: gap-versus-strain, for
             instance, enters only through row 63's separate deformation
             potential, never through the frozen G₀W₀ shift. This is a
             deliberate trade — it is what makes the tier closed-form — and
             its cost is a **dressing-staleness** term in the error model
             (`residual-definitions`). What is NOT yet declared is the
             **validity radius**: how far the state may excursion before the
             frozen dressing stops being a fair approximation. Until it is,
             the staleness term has no bound, and a composition cannot be
             refused for leaving the radius because no radius is stated.
...
Layer 1.75   Iterative fixed-point dressing — DEFERRED to V2 in code,
             ...
             Cert: IterativeResult (residual-machinery §7).
...
- **First-order SCP (`SCP-perturbative`)** is judged and *not* wired:
  ...
  `capability-slices`), and the full self-consistent SCPH (row 13)
