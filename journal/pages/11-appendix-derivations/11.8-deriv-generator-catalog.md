...
> rows ([formula-registry] has the band map). The §3 `ResidualGenerator` record predates
...
| 41 | vibronic-coupling-factor | `(Q_def, ω_λ) → S_HR` | B4/B2 | T2 | D2 | faithful | S1 | Q_def, phonons |
...
| 74 | field-activated-ionization-rate | `(α0, β/E) → α(E)` | B9 | T0 | D1 | cheap | S4 | E_field |
...
28. `field-activated-ionization-rate(α0_diamond, β/E)` → α at 10⁶ V/cm  [#74, T0/D1]
