...
`StateComponent` is the existing 7-tuple alphabet (`unified-state`).
...
(§10) records whether the channel's mediating interaction is
...
When it is only a part, `kernel_extension` (§11) carries the
...
record InvariantTerm {
  ...
  symbolic-form   : SymbolicTensor             -- the explicit term; stored as the
                                               ...
                                               -- TypedLeaf] per representation-substrate §2
...
non-polynomial kernel (§10–§11):

```
record GeneratorOutput {
  polynomial_invariants : List<InvariantTerm>     -- the symmetry-generated basis
  polynomial_sufficient : Bool                     -- echoed certificate (derived, §10)
  kernel_extension      : Optional<KernelExt>      -- the non-polynomial remainder, if any (§11)
  gauge_rule            : Optional<GaugeRule>       -- a basis-fixing rule, if any
  output_hash           : Address[GeneratorOutput]  -- domain-separated; folds in all four above
}
```
...
that Stage 2 already uses *decompositionally* (`compose-time-pipeline §2`
...
generate-invariants(G, c) :
  -- (0) well-formedness (§10): the flag and the kernel must agree
...
(`representation-substrate §4`), so per-composition cost is one-shot.
The cache key does **not** include the theory context (§11): the
...
> `polynomial_sufficient` question (§10), not the emptiness question.

## 4 Worked example — diamond electron-phonon
...
2. **Stage 2** has already constructed the diamond symmetry group
   (Fd-3m + time-reversal).
3. **Stage 2.5** (new sub-stage, §5) runs
...
## 5 Stage 2.5 — invariant synthesis

A new sub-stage in `compose-time-pipeline §2`, executed between Stage 2's
...
- **Kernel extensions add as one more summand.** When a channel carries
  a `kernel_extension` (§11), its lowered kernel node adds into the
...
The invariant-generator structure simplifies **three** cert obligations —
1 (symmetry equivariance), 2 (PSD of M) and 5 (antisymmetry of L) — which
`cert-obligations` §1.1 collapses to projection-residual checks:
...
- **The symmetry-equivariance obligation.** Polynomial invariants are
  ...
  little-group of q" (`KernelExt.symmetry_law`, §11), so cert checks
...
- **The positivity obligation** (antisymmetry of `L` / PSD of `M`).
  ...
  output matches the emitted `symbolic-form` within `δ_sym` (`cert-obligations §1.2`).
  For `PSDSymmForm` channels, PSD *existence* is a structural theorem rather
  than a runtime search — see the documented assumptions in §12 — and the
  ...
  kernel, via `cert-obligations §1.1` obligation 2.

The polynomial checks are O(1) per invariant; both are integrated with
`SymmetryAdaptedHamiltonianOf` (`canonical-vocabularies §2`) which
already lives in the symmetry machinery. The cert-obligation indices are now
fixed in `cert-obligations §1.1`: equivariance = obligation 1, antisymmetry of `L` =
...
generators (`residual-machinery §3`):
...
`named-formulas`). The channel's identity is `Address[CouplingChannel]`
under the canonical-serialization rule of `representation-substrate §4`
...
`ProvenanceLedger` (§4 caveat). Each provenanced coefficient must carry:
...
reference-battery `σ` machinery, `cert-obligations §2`). A **cert obligation refuses any
...
whose active `P_sp` and `e₃₁` carry mismatched tags** (`cert-obligations §1.3`). The ±5%-ΔP target also
...
```
record CouplingSpec {
  channels       : SparseSet[CouplingRegistry]   -- the active channels (representation-substrate §3)
  theory_context : TheoryContext                  -- the global theory frame (§11)
}
```

`CouplingSpec` was previously a bare `SparseSet[CouplingRegistry]`; it is
now a two-field record. Its `Address` is computed by the
`representation-substrate §4` record rule, so two specs with identical
...
`CouplingSpec` schema version is **bumped** (`representation-substrate §9`) so old
bare-set addresses cannot collide with new record addresses. The spec is
carried alongside the composition request (`compose-time-pipeline §1`). The
...
(in `M`), under the MVP default theory context (§11).
...
## 9 Coverage policy (not a hand-curated registry)
...
The invariant generator (§3) is the filter that culls structurally
...
§10 — which now includes the **piezoelectric acoustic** channel
(`LongRangeStatic(1)`, `1/q` pole) alongside the Fröhlich (`1/q²`) one, the second
long-range e-ph mechanism the wurtzite III-N members carry (`is-noncentrosymmetric`-gated — piezoelectric scattering needs a piezoelectric class, the applicability-classifiers split;
inert for diamond). This **closed the former open-decisions coupling-channel coverage-policy item** (now recorded under `[open-decisions]` Closed decisions).

## 10 Mechanism range and polynomial sufficiency
...
`applicability` (§2) independently says "this mechanism is active for
...
## 11 Extension types — `KernelExt`, `GaugeRule`, `TheoryContext`
...
onto the `representation-substrate §1` primitives.
...
`Address[TabulatedField]` (`representation-substrate §6`) — folded into the channel's
...
**`TheoryContext`** is the global theory frame on `CouplingSpec` (§8).
...
record TheoryContext {
  xc_functional          : XCFunctionalTag                         -- closed C1 vocabulary (canonical-vocabularies §7)
...
defined in `canonical-vocabularies §7`. The
...
therefore solely metadata for the cert + provenance layer (§7).
...
reference-battery obligation (§7), not by upgrading the default;
`Hybrid(HSE06)` is the documented accuracy upgrade for gap-sensitive work.

## 12 PSD closure for `PSDSymmForm` channels
...
so the positivity obligation (§7) never runs a semidefinite-feasibility
...
super-block (`cert-obligations §1.1` obligation 2; tolerances valued in `cert-obligations §1.2`).
