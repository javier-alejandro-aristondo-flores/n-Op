...
- **Implementation language (H1) — resolved.** The concrete needs (reverse-mode
  ...
  **polyglot of DSLs** — a compiler host for Stages 1–4 + the substrate that
  *emits source* for a separate runtime host (which owns the optional GPU
  codegen), plus an offline group-theory engine and an offline proof assistant.
  **That four-role shape is the settled decision; the languages filling the
  roles are not** (`open-decisions`; `[deriv-language-study]` records
  the leading candidates and why).
...
- **Design-grade accuracy targets (H8).** The MVP's headline outputs must meet
  ...
  ±0.2 eV, μ factor-2 (full per-observable ledger in [accuracy-ledger],
  wired via `residual-definitions §7`). Cert obligation 4 checks them at the
