...
substrate's sense (`representation-substrate §2`, §3 row for
...
## 1.1 Coupling-derived simplifications (obligations 1, 2, 5)

When a formula node originates from the invariant generator
(`coupling-structure §3`), obligations 1, 2, and 5 collapse to
projection-residual checks (tolerances named in §1.2):
...
- **Obligation 5 (antisymmetry of `L` — a conservation property).**
  ...
  conserves energy; Jacobi status per `generic-dynamics`.)
- **Obligation 2 (PSD of `M` — a bounds/positivity property).** For
  ...
  super-block. PSD *existence* is the structural theorem of `coupling-structure §12`;
  this is its runtime guard.

These checks are `O(1)`–`O(block)` per invariant and run alongside the generator
at Stage 2.5.

## 1.2 Tolerance ledger

Canonical names and default values for every tolerance / error bound in `/physics`.
`ε` is reserved for permittivity in the physics formulas (this ends the `ε`
collision noted in `coupling-structure`).

**`τ` is not a reserved tolerance prefix, and this page used to claim it was.**
It said "`δ` / `τ` denotes a *tolerance* throughout", which is false of the
appendices: `τ_n` and `τ_p` are SRH carrier lifetimes, `τ_PO` a polar-optical
scattering time, `τ_E` an energy relaxation time, and `τ_hop`, `τ_iv`,
`τ_alloy` likewise — 48 uses across six pages. **A `τ_x` is a tolerance only
if it appears in the table below**; everywhere else read it as a physical time.
The overclaim mattered: it made an exhaustiveness checker look sound, and that
checker fired 48 times on correct prose before the premise was examined
(`[traps]` §69). Separating the two mechanically would need a namespace the
corpus does not have — `tol_adj` versus `τ_adj` — so exhaustiveness here stays
a review rule, and is now labelled as one rather than implied to be checkable.

These values are the inputs `typeclass-alphabet` `Quantity.combineTol`
composes into the per-observable error budget (`residual-definitions §7`).
...
| `δ_meta` | T,P-hull metastability band (`residual-definitions §1` item 17, registry row 124) | `50 meV/atom`, per-material overridable (diamond +25 reads R=0) |
| `τ_adj` | registration adjoint vJp-vs-JvP gate, `D2` and `D3` (`residual-machinery §5`) | `1e-4` relative |
...
| `τ_trunc` | a-posteriori estimate of the gradient error a **truncated** inner solve introduces in an implicit-function adjoint — the error `τ_cond` cannot see, because `τ_cond` bounds the Jacobian's conditioning while `τ_trunc` bounds `‖J⁻¹‖·‖r_stop‖`, the product of that conditioning with the stopping residual. Emitted by the fidelity generator (`residual-machinery §4.1`), not a threshold to configure. Computable a-posteriori estimates of this shape — (inverse-Jacobian bound) × (stopping residual): Ehrhardt & Roberts, *IMA J. Appl. Math.* 89(1) 254–278 (2024), Thm 9. **The transfer is by analogy, not directly**: they assume a μ-strongly-convex lower-level *optimization* problem, whereas an SCF inner solve is a general nonlinear fixed point. Blondel et al. (NeurIPS 35, 2022) §3.4 bound the same Jacobian error for a general optimality condition, `‖J(x̂,θ) − ∂x*(θ)‖ < C‖x̂ − x*(θ)‖` — closer in setting, but *not* a-posteriori computable, since it is stated in terms of the unknown `x*` | measured per instance; enters `combineTol` |
...
| `δ_plan` | Stage-4 per-compression-plan truncation error target (`compose-time-pipeline §4`); the sum over active plans is the compression term in `combineTol` | per-plan, declared at plan selection |
| `τ_NEB` | `PathStationaryOf` climbing-image NEB force convergence (`typed-compositions`) | `1e-3` |
...
in the other page — but nothing enforces that. `check_data_agreement.py` check (c) compares a
...
## 1.3 Composition-validity refusals (machine-checkable, not reviewer caveats)
...
- **Unprovenanced-coefficient refusal** (obligation 4/9 family, `coupling-structure §8`).
...
- **AHC `slope-kind` double-count refusal** (obligation 6, named-formula consistency).
  ...
  (`coupling-structure §8`). A composition in which a `total`-tagged AHC slope and row 63's
...
- **Learned-correction-without-anchor refusal** (obligation 9, surrogate validity). A PINO-learned
  correction coefficient (V1: the EDF-tail `Δα`, `coupling-structure §8`) is admissible only if external
...
- **Polarization-convention pairing refusal** (obligation 6, named-formula consistency;
  `coupling-structure §8`). Each spontaneous-polarization (`P_sp`, row 113) and piezoelectric (`e₃₁`,
...
## 2 `SqliteReferenceCache` — backend for obligations 4 + 8
...
table entries (
  ...
  coverage_mask   BLOB  NOT NULL,      -- RoaringCoverageMask serialization (pino-bridge §2.1)
...
- **Key construction.** `ContentAddress` over the canonical serialization
  ...
  backed, per `representation-substrate §4`). Identical payloads
...
The cache and the **EvidenceDAG** (`[representation-substrate]` §3, cluster C4) are the
