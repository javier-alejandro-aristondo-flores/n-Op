...
- **F-G2 `hydrogen-redistribution-drift-diffusion`** — `([H](r),T,E,μ_drift) → ∂[H]/∂t`,
  ...
  H interstitial `E_diff=1.7 eV`, `D(500 °C)≈1e−13 cm²/s` (~1 mm profile shift in 1000 h). T3/`D2` (row 106 — the output is a rate field from a transport operator, not a converged fixed point);
