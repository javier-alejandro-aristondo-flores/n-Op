---
id: product
title: n-Op — Product Behavior (`/physics` as a deliverable)
authority: canon
content-hash: 5f663da41443
canonical-for:
  - product
depends-on:
  - accuracy-ledger
...
> [computational-overview] and [accuracy-ledger] — held to the same citation
> rules as any other page by the checkers; every claim cites its canonical `arch-xx` / `impl-xx` source
...
*consumer*, not the product (`library-landscape`); every loop that drives either of them
lives outside both, in `/interface` (`purpose-and-scope`).
...
**Consumers.** Two classes, one surface (`pino-bridge`):
...
- **Evidence, never verdicts.** No aggregation, thresholding, or judgment anywhere in the
  product (`residual-definitions`).
- **Refusal is first-class.** What the oracle cannot stand behind is absent, and the
  absence is accounted for — never papered over (`cert-obligations`).
...
- **Score, not solve.** The caller supplies complete candidate states; the oracle never
  fills in missing pieces (`purpose-and-scope`).

## 2. Deployment shape: a compiler and its oracle-files

The product has two parts, matching the architecture's two phases (`compose-time-pipeline`,
[computational-overview]):
...
1. **The callable** — `Validate` (§4), with its gradient entry point baked in at compile
   time (`pino-bridge §1`). Consumers that never need gradients simply never
...
2. **The static slot schema** — the complete table of every check the kernel contains:
   ...
   no other resource.
3. **Its identity** — the content hash of the compiled kernel (`representation-substrate
   §4`), the identity descriptors it was compiled from, and the registry/spec versions
...
4. **Its certificate reference** — a hash-pinned pointer to the certification evidence
   for this kernel (`cert-obligations`), so trust travels with the file.

**Behavioral consequences, stated as rules:**

- **One file per crystal identity.** A kernel is specialized to one
  `(PeriodicityStructure, SiteDecoration, Environment)` tuple (`compose-time-pipeline §1`).
...
- **Environment-box validity.** The compile-time environment fixes the *envelope* — which
  checks exist — and stamps the file with its validity box (`applicability-classifiers`);
...
how it is decorated, the operating conditions (`crystal-inputs`). Its semantic model is
...
**State (to the oracle-file, per call).** The full state object as `unified-state`
...
*physical* admissibility is scored, never presupposed (`residual-definitions`).
...
renderable as a hierarchical path (`residual-definitions`). The residual value is **raw**:
...
dressing — `residual-machinery`, for subset selection and grouping only), and the
error scale σ (`residual-definitions §7`). Consumers who want cross-slot comparability
...
(`cert-obligations`). No prose, anywhere.
...
- **Compose-time scoping.** The compile request names the observable bundles and residual
  categories wanted (`compose-time-pipeline §1`, vocabularies of `canonical-vocabularies`);
...
- **Call-time subsetting.** `Validate`'s `request` parameter evaluates the full kernel, a
  set of slot keys, or a set of named quantities (`pino-bridge §1`).

*Recorded refinement (future `pino-bridge` edit, deliberately not made yet):*
...
(`pino-bridge §2`). Two readings of the same object:
...
(`pino-bridge §2`). New pins → new oracle-file. That is a feature: every
...
both loops live in `/interface`, never in the product (`purpose-and-scope`).
...
external search over that discrete space. Static, instantaneous-property design works on
today's contract;
...
- `inspect` — oracle-file → its static schema and identity, enumerated (the internal
  `EnumerateObservables` made visible; `pino-bridge §3`).
...
- It holds no state values, never evolves anything, never trains, and owns no loop
  (`purpose-and-scope`, `library-landscape`).
...
- It accepts no open-ended expressions: the grammar is the closed vocabularies of
  `canonical-vocabularies`, and requests either compile or are rejected at compose time.
- It emits no natural language.

## 9. Open decisions and unclaimed capabilities

Recorded here in the manner of `open-decisions`; none blocks the v1 verbs
...
1. **Time-evolution verbs (rollout; endpoint/lifetime queries) — still unclaimed; the
   ...
   (`live/specs/2026-07-16-evolver-duality-research.md`) resolved *survives with
   ...
   `open-decisions`). §8 is preserved verbatim: the hand-off is tendencies,
...
3. **Wire formats.** Serialization of state files, residual/value maps, and the schema
   ...
   `representation-substrate §4`; otherwise open.
