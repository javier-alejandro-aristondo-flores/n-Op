...
Closed registry of typed, fully-parameterized algebraic formulas. The registry is named-by-behavior; person-attribution names appear only as parenthetical pointers in the Source column for literature trace-back. Canonical machine-readable list at `physics/library/formulas/registry-manifest.csv` (102 substantive rows + 2 architectural markers as of γ''). Categories include: slab-arithmetic, exponential-rate-law, harmonic-transition-rate-normalization (ratio of products-over-modes at minimum vs saddle), causal-response-pairing (real ↔ imag via Hilbert transform), hardness-from-elastic-moduli, elastic-tensor-averages, sound-velocity-from-elastic-tensor, bosonic-thermal-occupation, fermionic-thermal-occupation, defect-formation-energy, electronic-thermal-conductivity-from-electrical, photon-emission-vs-absorption-balance, transition-rate-from-saddle; plus the UWBG-focused additions (polar-optical-mode coupling, polar-optical-mode-limited transport, deep-trap nonradiative recombination, field-enhanced impact-ionization, field-tunneling, field-saturating mobility, metal/semiconductor band-offset with interface-induced-states correction, surface-tunneling current, charged-supercell extrapolation in four schemes, composition-dependent excess-free-energy basis, and others).
...
A meta-audit identified several apparent gaps in the closed registries — long-range-Coulomb directional-limit correction for polar materials, charged-supercell finite-size extrapolation (three or four distinct scheme parameterizations), internal-strain coupling, self-consistent temperature-dependent vibrational renormalization, composition-dependent excess-free-energy basis (continuous-composition phase diagrams), cycle-basis reduction for detailed-balance enforcement in rate networks. Three named **paths** for closing these were considered.
...
- The long-range-Coulomb directional-limit correction for polar materials is **not** automatic; the closed-form directional-limit term is a real formula entry, plus the small-dual-index directional limit is a cert obligation. Net: +1 formula + 3 Layer 1 primitives (effective-charge tensor, high-frequency response tensor, electronic linear-response tensor).
- The four charged-supercell extrapolation schemes are **not** the same method with different coefficients. They compute distinct integrals over distinct domains: one uses planar-averaged potentials with alignment, one uses atomic-site averaging, one is the isotropic asymptotic series, one uses image-charge with screening. They should be modeled as a strategy-pattern method `charged-supercell-extrapolation(scheme: {isotropic, planar-aligned, atomic-site-averaged, image-charge}, …)` plus a separate lattice-Coulomb-scalar Layer 1 primitive — **not** merged.
...
A separate research document on the symmetry-indicator classification framework for crystalline operators was reviewed and identified as **directly load-bearing** for several pieces that are vague or flagged-as-gap in the architecture above.

Specifically: `(SG, AZ_class) → AbelianGroup X_BS = {orbit-induced reps} / {trivial reps}` computed in polynomial time via Smith Normal Form on the orbit-representation matrix; 117 of 230 space groups have non-trivial X_BS under TRS in the spin-doubled setting; max |X_BS| = 72 at SG 175 and 191. Resolves the open "ten-element symmetry-class label" classification gap. A lookup table indexed on `(X_BS_generator, boundary_orientation) → boundary_mode_count` converts cert obligation-7 (bulk-boundary correspondence) from a stub to a derived assertion. Tool ecosystem (Crystalline.jl, MagneticTB, IrRep, Z2Pack, WannierTools, the crystallographic-tables databases) creates concrete language-decision pressure: Julia or Python clearly viable; Mathematica only viable as a partial-precompute supplement.

If integrated: adds `SymmetryAdaptedHamiltonianOf` template; adds a symmetry-classification Layer 1 / Layer 1.25 primitive; substantively closes cert obligation-7; folds `topology-resolved` into the existing `electronic-structure` bundle (rather than adding a 12th bundle).

**Status (γ''):** integrated. See `IMPLEMENTATION-PLAN.md` §28.

A flagged but un-pursued cross-pollination opportunity: framing classified-operator synthesis through dependent type theory. `(SG, orbits, dimension, target-X_BS)` becomes a type; emitting an operator realizing that classification becomes type inhabitation / proof search. Possible novel CS contribution. Not in V1 scope.
...
2. **[RESOLVED, γ'' §28]** Topology integration: included as the **navigational atlas** over composition space, not deferred. `SymmetryAdaptedHamiltonianOf` template added (§8 entry 15); X_BS computed in polynomial time via Smith Normal Form on the integer matrix of orbit-induced representations; the orbit-representation matrix, irrep-compatibility matrix, first-band integer invariant, binary-with-symmetry invariant, holonomy spectrum, and boundary-mode multiplicity-from-classification added as registry rows 96–102. Cheap parts always-on at compose-time; expensive global integrals over the dual-space grid are opt-in per observable. Cert obligation-7 (bulk-boundary correspondence) is filled by a lookup table indexed on `(classification_generator, boundary_orientation) → boundary_mode_count`.
