...
- **The data-structure layer — no longer open.** This bullet listed the four `γ̂` `§15.4`
  ...
  dispositions and `arch-15 §15.4` is canonical. Kept here rather than deleted because
  ...
  (`arch-18-open-decisions`, verifier-soundness gap 1). The registration gate validates
...
- **Deferred engineering decisions** (`arch-18`, open list) — surrogate-net build-vs-adopt, the
  PDE-mesh **adjoint scheme** (the mesh *format* is now committed in `arch-21`), the
