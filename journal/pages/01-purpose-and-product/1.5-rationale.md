...
- **A certified specification base.** 58 pages with explicit topic ownership, 99
  ...
  (`journal/tools/`) covering twelve structural invariants and sixteen mechanical sweeps, plus a third tool that calibrates them — 48 probes, each planting a defect and asserting it fires, plus a gate that refuses green if any check has no probe at all —
