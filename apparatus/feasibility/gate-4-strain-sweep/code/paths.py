...
#: Repository root. `code/` -> gate -> `feasibility/` -> `apparatus/` -> root.
...
RUN_MANIFEST = REPO / "apparatus" / "data" / "diamond-strain-sweep" / "index-of-all-runs.tsv"

# So `programs.oracle.registry.elastic` imports whether or not this is run from the repository
