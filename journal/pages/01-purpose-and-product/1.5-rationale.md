...
operator can be disciplined by it at every step ("score-not-solve", `arch-01-purpose`,
`arch-16-pino-bridge`). The oracle never solves for anything — the operator supplies the
...
- **A certified specification base.** 58 pages with explicit topic ownership, 99
  ...
  obligations with four explicit refusal modes (`arch-09`, `arch-12`). On 2026-07-16 the
...
- **A concrete first build.** The MVP is diamond-first and carved out precisely: three
  ...
  written build order (chapter 8, `mvp-06-build-order`).
