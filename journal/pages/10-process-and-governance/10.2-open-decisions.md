...
3. **No declared validity radius for frozen Layer-1.25 dressings.** The
   ...
   it. `OneShotCert` (`[impl-07-residual-factory]` §7.7) carries no
   validity-radius field to put one in. Related and also unrecorded: **G₀W₀ cost
   is not scope-tagged by cell size.** At MVP scale a T2 budget (≤10 s) is
   plausible; for V1 defect supercells it is hours, so the tier table is violated
   silently rather than refused.
4. **Gradient error under truncated inner solves is unbudgeted.** The
   implicit-function adjoints of `D3` rows assume the forward solve *converged*.
   `τ_cond` now guards the fixed-point Jacobian's conditioning at registration,
   but nothing budgets the error a truncated inner solve introduces at runtime —
   an SCF stopped at its tolerance, or the L3↔non-equilibrium cycle stopped at
   its ≤5-iteration cap. The conditioning guard and the truncation budget are
   different things and only the first exists.
5. **No numeric per-observable mesh-σ floor.** `[accuracy-ledger]` row 2
   referred to "the declared mesh-σ floor" from 2026-06-10 until 2026-07-21
   without one ever having been declared. What is established is the
   quantification, now recorded there: BTE transport needs ~50³-equivalent
   k-sampling, the MVP's 8×8×8 / 29-IBZ mesh is factor-2-grade, and IBZ
   reduction helps cost rather than convergence. The floor itself is open.
