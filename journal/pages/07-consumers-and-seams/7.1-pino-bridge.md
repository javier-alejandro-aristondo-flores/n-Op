...
## 1 `Validate` — the differentiated residual surface

```
Validate(state    : UnifiedState,           -- the 7-tuple of unified-state
         env      : Environment,
         request  : all | {ResidualKey} | {ObservableRef},
         gradient : Skip | Compute)
       → ( residuals : Map<ResidualKey, Scalar>           -- granular per residual-definitions
         , values    : Map<ObservableRef, Value>          -- bundled observable outputs
         , cograds   : Optional<Map<ResidualKey, Cotangent>>  -- the kernel's
                                                              -- `gradient` map
                                                              -- (residual-definitions §4)
         , cert      : CertEvidence )
```
...
`ResidualGenerator` (`residual-machinery §1`). At Stage 1
(`compose-time-pipeline §1`) the generator inserts a pinned `Input`
...
4 (reference battery, `cert-obligations`) and feed `/informed-operator`'s
target-vs-prediction comparison.

### 2.1 `CoverageMask = RoaringCoverageMask`
