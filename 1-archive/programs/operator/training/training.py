...
# Inside a section directory, the library root is not on the path when this file is
# run directly. Same one-line fix `models/` already uses.
if str(Path(__file__).resolve().parents[1]) not in sys.path:
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))

import locations  # noqa: E402, F401  -- puts Gate 4's code on the import path
