...
**Recommended reading order (token-efficient):** [computational-overview] → `glossary.md` → the architecture atomic files in this priority: 05, 20, 07, 06, 09, 19, 11, 04, 15, 08, 12, 10, 13, 14, 16, then 01/03/17/18 → the `physics/research/*` group files (these are the physics ground truth — compare them against what landed in the spec) → `implementation/*` and `mvp/*` for the build/accuracy commitments → `physics/library/formulas/registry-manifest.csv` for the formula inventory.
