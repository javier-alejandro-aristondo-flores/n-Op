---
id: deriv-language-study
title: Implementation-language decision (`open-decisions`, open item 5)
authority: supporting
content-hash: 50fe2cd02d72
canonical-for:
...
referenced-by:
  ...
  - open-decisions
---
# Implementation-language decision (`open-decisions`, open item 5)

> **Status — partially superseded.** What closed in 2026-06 was the *shape*: a
> four-role polyglot (compiler host → emitted source → runtime host, plus an
> offline group-theory engine and an offline proof assistant), joined at the
> Stage-4→Stage-5 codegen seam. **The language picks below did not close.** Read
> Haskell / Julia / GAP / Lean 4 as this study's leading candidates and, more
> usefully, as a record of *which requirement each was chosen to satisfy* — that
> is the part a later comparison should carry forward. [open-decisions]
> is canonical for the current status.
...
The prior framing (`open-decisions §1`: Julia vs Python+JAX vs custom-MLIR) was "which
...
| Section (`compose-time-pipeline`) | Language | Role | Live / offline |
|---|---|---|---|
| Stages 1–4 — symbolic-IR compiler + `representation-substrate` substrate | **Haskell** | typed IR, e-graph, lowering, adjoint synthesis, **emits the kernel** | live (compile-time) |
...
Haskell hosts Stages 1–4 and the `representation-substrate` substrate. It is the only candidate that makes
...
- **A-la-carte capability typeclasses** (`typeclass-alphabet`: `Sampleable` + optional
...
array in / residual + gradient + observable + cert arrays out. **No `representation-substrate` substrate
object ever serializes across** — the substrate lives entirely compiler-side; Stage 5 is
"no symbols" (`compose-time-pipeline §4–7.5`). That narrowness is the safety argument for the split.
...
the `O(log n)` / no-solver-on-the-hot-path commitment (`representation-substrate §5`). Julia owns the
...
- **2026-07-21 (partial reopen):** the language *picks* are open again — only the
  four-role polyglot shape stayed closed. [open-decisions] open item 5 carries
  the reopened part; the picks below are candidates, and the durable content of this
  study is the requirement each candidate was chosen to satisfy.
- **2026-06 (decision closed):** the recommendation here was adopted (commit ec52314-era) —
  ...
  in open-decisions "Closed decisions"; this file remains the research record behind it. Superseded
  in part by the 2026-07-21 entry above.
