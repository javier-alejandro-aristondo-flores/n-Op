...
| 31 | `schottky_barrier_height φ_B(metal/semi, T)` | SC | NEGF or DFT slab + alignment; **interface-MIGS calc may be OUTSIDE registry if NEGF not present** → landed: registry row 80 (NEGF-transmission) | `φ_B = φ_M − χ_S − (1 − S)(φ_M − φ_CNL)`, equivalently `φ_B = φ_CNL + S(φ_M − φ_CNL) − χ_S` (Cowley-Sze interpolation; **S=1 → Schottky-Mott, S=0 → Bardeen/fully pinned**, matching [deriv-defects] §F.3. The `S(φ_M − φ_CNL)` form inverts it) | (a) Schottky-Mott `φ_B = φ_M − χ`; (b) Cowley-Sze with tabulated S | `r_φB = φ_B − [φ_CNL + S(φ_M − φ_CNL) − χ ± dipole correction]` from slab | ±0.1 eV (critical: 100 meV → ×e^4 contact resistance at 500°C) | Interface chemistry (carbide formation) shifts φ_B with thermal soak | 7, 8, 30, FoM ρ_c |
