...
substrate-agnostic reference oracle (`library-landscape`): it defines what a
...
(`pino-bridge`) — states and cotangents in, keyed floats out. So the
...
7-slot state tuple exactly as `unified-state` defines it — slot set, per-slot
...
`representation-substrate §3`, not by ad-hoc names). **Structural**
...
`residual-definitions §1`; the score-not-solve principle of
`purpose-and-scope`). Emitting a structurally-valid but physically-poor state is
...
time (`compose-time-pipeline §2`). Given per-channel lists of query coordinates,
...
*per-key* cotangents (`pino-bridge §1`, `cograds`); the external
...
time) as named, typed conditioning channels (`crystal-inputs`). Emitted state
...
float normalization of `representation-substrate §4` (canonical quiet-NaN,
...
with the `representation-substrate` addressing discipline, so any residual map
...
search — lives in `/interface` (`purpose-and-scope`). No control inversion, no
required callbacks.
...
actions (`compose-time-pipeline §2`). If `Learnable_Structure` can consume a
...
(`multiscale-state`). If the kernel supports per-slot update cadences
