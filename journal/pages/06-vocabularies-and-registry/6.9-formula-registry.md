...
`PhysicsGraph` (see [physics-graph]); the registry is the
...
- **132 substantive formulas** (rows 1–102 + 105–134). Rows 105–112 are the
  slow-tier degradation / radiation formulas (`multiscale-state §14`);
  rows 113–115 and 117–118 are the polarization / piezoelectric / 2DEG package
  (`is-noncentrosymmetric`-gated — the piezo-class predicate of the applicability-classifiers
...
| `Bundle` | Observable bundle membership, `B1`–`B11` or the `L1` primitive tag (`canonical-vocabularies §4`, `named-formulas`). |
| `Tier` | Cost tier `T0`–`T3`. |
| `Diff` | Differentiability tag; vocabulary owned by `[named-formulas]`. |
| `Path` | **Anchor class**, `cheap` or `faithful` — what the row's value is trusted against, *not* a runtime path selector (every formula is on the single residual surface under the always-cheap pipeline, see [compose-time-pipeline]). It is the axis obligation-6 consistency pairs run along. Semantics owned by `[named-formulas]`. |
...
**Differentiability** — see `[named-formulas]`, which is canonical for the
...
[residual-machinery]). The tags drive everything downstream:
...
to `/informed-operator` (see [residual-definitions]).
