...
the **III-N flagships (GaN, AlN, AlGaN)** — fully sourced and adversarially audited; the
seeding spec and the audit that refuted it both collapsed into
`[timeline] §2026-06-10 (Wave 1)`, whose `[Wave-1 A#]` table is the correction record — plus
the diamond anchors already in
[accuracy-ledger]. **Polarization e₃₁ is the PROPER constant, paired with the ZB-reference
P_sp** (the self-consistent convention enforced by `cert-obligations §1.3`; never mix with improper e₃₁).
**Population status (2026-07-08, gap-audit Phase 3).** The **full diamond MVP battery is
seeded** (`forced-decisions §H4`: lattice a, indirect gap, C₁₁/C₁₂/C₄₄ + bulk modulus + density, Θ_D,
...
best-exp cells on 2026-07-16). Remaining `—` cells are the explicit GAP rows, the AlGaN interpolation-rule row, the GaN `breakdown-field-slope-kBR` row (a device `dV_BR/dT` whose normalized K⁻¹ form is itself a declared GAP), and
...
- **Uncertainty (σ)** — one-sigma uncertainty band (instrumental or computational). Three encodings
  ...
  coefficient until a σ-assignment pass values it (`coupling-structure §8` requires `(value, σ, source,
  cost-class)`; `cert-obligations §1.3` refuses compositions carrying unprovenanced coefficients).
