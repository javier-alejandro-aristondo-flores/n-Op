...
def Identity_And_Affine_Floors(
    pairs: Sequence[FunctionalPair],
    load: Callable[[str], Field],
    ...
    slopes: list[float] = []
    for pair in pairs:
        cheap = load(pair.cheap_identifier).ravel()
        accurate = load(pair.accurate_identifier).ravel()
...
def Scissor_Floor(
    ...
    for pair in pairs:
        gaps: list[float] = []
        for identifier in (pair.cheap_identifier, pair.accurate_identifier):
            with np.load(Archive_Path(campaign, identifier, pool_root)) as archive:
