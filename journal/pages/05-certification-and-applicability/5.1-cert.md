...
This table is **exhaustive by construction**: `seams.py` sweeps the editable
surface for tolerance symbols and reports any that diverge between sites, and a
tolerance stated anywhere in the corpus but absent here is a defect in this
table, not in the other page. The claim above is only as good as that sweep —
it was false when written, omitting six entries, because no sweep existed.
