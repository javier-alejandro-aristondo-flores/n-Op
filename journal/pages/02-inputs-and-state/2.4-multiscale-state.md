...
*homogenized coefficients* the micro tier supplies (macro, §21.8). In `born-oppenheimer-levels`
...
A closed C1 vocabulary `DefectSpecies` as a `Universe[T]` instance (`representation-substrate §1, §20.3`;
...
Eight of the nine processes below are new `FormulaRecord`s (rows 105–112); **F-F5 is the existing row 81**, re-tagged so its output `x_carbide` is a slow-state field (§21.13). all Arrhenius rates use `rate = ν₀·exp(−E_a/kT)`
...
weightable `ResidualLeaf` per `(species, charge, site)` (`residual-definitions §2–§11.3`), no
...
`unified-state` emergence axiom refined (§21.0); `generic-dynamics` per-tier generators (`[generic-dynamics]`,
...
`EOM/Continuum` (17→19) and R-T1/R-T2/R-T3; `crystal-inputs` adds the §21.11 `Environment` fields;
