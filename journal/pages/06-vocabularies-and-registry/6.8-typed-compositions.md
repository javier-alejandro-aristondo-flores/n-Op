...
Conductivity        = { ConductivityViaBTE, ConductivityViaKubo }
                      (both formulas evaluated; method-equivalence is enforced
                       ...
                       residual — residual-definitions, cert-obligations)
...
slot, which `[named-formulas]` forbids outright. They now carry the declared
...
|---|---|
| `absorption-from-dielectric` | `α(ω) = (2ω/c)·Im(√ε)` — the `2` prefactor is the corrected form; `[named-formulas]` is canonical |
