...
per-stage sidecars (§4), produced by one stage and visible to later
stages per the stage poset (`representation-substrate §6`), never carried into the
runtime kernel.

`NodeKind` (§2) is the closed C1 vocabulary that discriminates the
typed payload sum; this is the substrate's primary closed-polymorphism
mechanism (`representation-substrate §6`). Sidecars (§4) are typed
...
- **Observables and residuals** are *roles* (§3), not kinds. The same
