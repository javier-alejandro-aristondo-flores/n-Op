...
The example table in §1 collects the standard predicates.
...
- **Row 116** — needs a *semiconductor-interface* predicate, and the classifier
  ...
  this is recorded here and in `[open-decisions]` rather than filled in
  with a classifier nobody has defined. Until it is defined, row 116 carries an
  always-true stub, which §1's V1 commitment permits.

### 1 Applicability examples (illustrative; every registry entry carries its own field)
...
(QHA window, the regime windows of `multiscale-state §8.1`, the `ω²≥0`
...
(`pino-bridge`); compose-time `Stage-2.5` structure decisions are frozen, but the
...
`CouplingChannel.applicability` (`coupling-structure §2`) uses
...
(`representation-substrate §2`, §3 row for the applicability
cluster): a reduced ordered Boolean DAG over typed parameterized atoms
drawn from the C1 typeclass-tag vocabularies (`typeclass-alphabet`).
The atom order is part of the predicate-vocabulary version
(`representation-substrate §9`); adding a new atom creates a new order id and forces
explicit re-canonicalization of stored predicate roots rather than
silent reinterpretation. Cert obligation checkers (`cert-obligations`) are
...
C2 generator-registry machinery (`representation-substrate §7`). The split is
preserved.
