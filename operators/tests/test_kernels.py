...
def Test_The_Dense_Kernel_Matches_The_Oracle() -> None:
    ...
    coefficients = Coefficients(vector=numpy.asarray([0.5, -1.0, 2.0, 0.25]), domain=CUBE)

    def Pair_Kernel(targets: NDArray[numpy.float64], sources: NDArray[numpy.float64]) -> NDArray[numpy.float64]:
        target_indices = numpy.asarray(targets[:, 0], dtype=numpy.int64)
        source_indices = numpy.asarray(sources[:, 0], dtype=numpy.int64)
        return weights[numpy.ix_(target_indices, source_indices)]
    ...
    assert numpy.allclose(numpy.asarray(produced.vector), reference[:, 0], atol=1e-12)
    assert "weights" in kernel.Inspect()


def Fourier_Feature_Map(points: NDArray[numpy.float64]) -> NDArray[numpy.float64]:
    """A fixed feature map: a constant plus one wave per axis."""
    waves = numpy.cos(2.0 * numpy.pi * points)
    return numpy.concatenate([numpy.ones((points.shape[0], 1)), waves], axis=1)
...
def Test_The_Low_Rank_Kernel_Matches_The_Oracle_On_Points_And_Grids() -> None:
    ...
    assert numpy.allclose(numpy.asarray(produced.vector), reference.reshape(-1), atol=1e-12)
...
def Test_The_Spectral_Kernel_Matches_The_Dense_Oracle() -> None:
    ...
    assert numpy.allclose(numpy.asarray(produced.values), reference, atol=1e-10)
...
def Test_The_Spectral_Kernel_Transfers_Discretization() -> None:
    ...
    assert numpy.asarray(produced.values).shape == (1, 8, 8, 8)
    assert numpy.allclose(numpy.asarray(produced.values), reference, atol=1e-10)
