...
operator model developed in this repository — is *targeted at*. The nine
categories below define the **target scope**, not the implemented scope: they
are the buckets any quantity a user might ask for should fall into.

**They are not a claim that the registry can compute all nine today.** The
registry is canonical for what exists (`physics/library/formulas/registry-manifest.csv`,
indexed by [formula-registry]); [typed-compositions] carries the declared gap
of composition formulas that are invoked but unregistered. Category 6
(Magnetic) is the clearest case — it has no dedicated bundle, because `γ̂` is
mean-field and the UWBG scope did not force one — and several entries in the
other categories are targets rather than rows. Read this page as scope, and the
registry as inventory; where they disagree, the registry wins.

## Property → bundle map

Each property category projects onto one or more of the **canonical observable
bundles** `B1..B11` (defined in [canonical-vocabularies] §4`). The
...
| Property category | Canonical bundles (`canonical-vocabularies §4`) |
|---|---|
...
`B11`="topology atlas" — superseded by the `canonical-vocabularies §4` canon, where
`B11`=degradation.)
